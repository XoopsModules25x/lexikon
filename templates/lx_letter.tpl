    ...
    <{if $pagetype == '0'}>
        ...
        <{foreach item=eachentry from=$entriesarray.single}>
            <h4 class="term"><{$eachentry.microlinks}><a href="<{$xoops_url}>/modules/<{$eachentry.dir}>/entry.php?entryID=<{$eachentry.id}>"><{$eachentry.term}></a>&nbsp;<{if $multicats == 1}>
                <a class="catname_min" href="<{$xoops_url}>/modules/<{$eachentry.dir}>/category.php?categoryID=<{$eachentry.catid}>">[&nbsp;<{$eachentry.catname}>&nbsp;]</a></h4><{/if}>
            <{if $eachentry.definition }><div class="definition"><{$eachentry.definition}></div><{/if}>
            <{if $eachentry.comments }><div class="xsmall">[&nbsp;<{$eachentry.comments}>&nbsp;]</div><{/if}>            
        <{/foreach}>
