...
if (!$init) {
    ...
    if ($init == _MD_LEXIKON_OTHER) {
        ...
        $queryB  = 'SELECT entryID, categoryID, term, definition, uid, html, smiley, xcodes, breaks, comments FROM ' . $xoopsDB->prefix('lxentries') . " WHERE submit ='0' AND offline = '0' AND init = '$init' AND init != '#' " . $catperms
                   . '  ORDER BY term ASC';
        $resultB = $xoopsDB->query($queryB, $xoopsModuleConfig['indexperpage'], $start);
    }
